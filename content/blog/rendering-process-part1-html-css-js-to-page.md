...
description: "在上一篇文章中我们介绍了导航相关的流程，那导航被提交后又会怎么样呢？就进入了渲染阶段。这个阶段很重要，了解其相关流程能让你\"看透\"页面是如何工作的，有了这些知识，你可以解决一系列相关的问题，比如能熟练使用开发者工具，因为能够理解开发者工具里面大部分项目的含义，能优化页面卡顿问题，使用 JavaScr..."
date: "2024-01-13"
author: "Kerwin"
tag: "Browser"
---
...
> **2025更新说明**：本文最初编写时，Chrome的布局系统（LayoutNG）尚处于开发阶段。截至2025年，LayoutNG已完全落地并成为Chrome的默认布局引擎，带来了更清晰的输入输出分离和更好的性能表现。此外，CSS领域也涌现了诸多新特性：`@layer`（级联层）重新定义了样式优先级管理；`contain` 属性和 `content-visibility` 为渲染性能优化提供了新的手段；User-Agent Client Hints 正在逐步替代传统的User-Agent字符串。以下内容已根据最新进展进行更新。
...
![代码转换成页面](/images/browser/rendering-process-part1-html-css-js-to-page/code-transform-page.png)
...
![代码含义](/images/browser/rendering-process-part1-html-css-js-to-page/code-meaning.png)
...
![渲染流水线](/images/browser/rendering-process-part1-html-css-js-to-page/render-assembly-line.png)
...
![树结构](/images/browser/rendering-process-part1-html-css-js-to-page/tree-structure.png)
...
![DOM树构建过程](/images/browser/rendering-process-part1-html-css-js-to-page/dom-tree-process.png)
...
![打印document](/images/browser/rendering-process-part1-html-css-js-to-page/console-document.png)
...
![修改document](/images/browser/rendering-process-part1-html-css-js-to-page/update-document.png)
...
![CSS样式来源](/images/browser/rendering-process-part1-html-css-js-to-page/css-source.png)
...
![document.styleSheets](/images/browser/rendering-process-part1-html-css-js-to-page/document-stylesheets.png)
...
![标准化后的属性值](/images/browser/rendering-process-part1-html-css-js-to-page/standard-attribute-value.png)
...
![DOM节点效果](/images/browser/rendering-process-part1-html-css-js-to-page/dom-node-effect.png)
...
![继承来源](/images/browser/rendering-process-part1-html-css-js-to-page/inherit-source.png)
...
> **2025补充：User-Agent Client Hints**：传统的User-Agent字符串（如 `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36...`）冗长且容易被用于浏览器指纹追踪。Chrome正在推进**User-Agent Client Hints（UA-CH）**作为替代方案。UA-CH通过结构化的HTTP请求头（如 `Sec-CH-UA`、`Sec-CH-UA-Platform`、`Sec-CH-UA-Mobile`）按需提供浏览器信息，服务器只能获取必要的信息，而不是整个UA字符串。这在保证功能的同时减少了隐私泄露。开发者应逐步从解析UA字符串迁移到使用 `navigator.userAgentData` API。
...
> **2025补充：@layer（级联层）**：CSS引入了 `@layer`（级联层）特性，为开发者提供了**显式控制样式层叠优先级**的能力。在此之前，管理大型项目中的CSS优先级常常是一场噩梦，开发者不得不依赖选择器权重、`!important` 或人为约定来解决冲突。
>
> 使用 `@layer`，你可以将样式组织到不同的层中，并明确声明这些层的优先级顺序：
>
> ```css
> /* 声明层的优先级顺序：reset < base < components < utilities */
...
> ```
>
> 后声明的层具有更高的优先级，这让CSS的优先级管理变得清晰可预测。`@layer` 对于大型项目、设计系统和第三方CSS库的集成尤其有价值，它是CSS层叠机制的一次重大进化。

总之，样式计算阶段的目的是为了计算出 DOM 节点中每个元素的具体样式，在计算过程中需要遵守 CSS 的继承和层叠两个规则。这个阶段最终输出的内容是每个 DOM 节点的样式，并被保存在 ComputedStyle 的结构内。

如果你想了解每个 DOM 元素最终的计算样式，可以打开 Chrome 的"开发者工具"，选择第一个"element"标签，然后再选择"Computed"子标签，如下图所示：

![计算样式](/images/browser/rendering-process-part1-html-css-js-to-page/computed-style.png)
...
![布局树的构造过程](/images/browser/rendering-process-part1-html-css-js-to-page/layout-tree-process.png)
...
在执行布局操作的时候，会把布局运算的结果重新写回布局树中，所以布局树即是输入内容也是输出内容，这是布局阶段一个不合理的地方，因为在布局阶段并没有清晰地将输入内容和输出内容区分开来。针对这个问题，Chrome 团队已经完成了布局代码的重构，新一代布局系统 **LayoutNG** 已经在 Chrome 中全面落地并成为默认布局引擎。

> **2025更新：LayoutNG已全面落地**：LayoutNG 已经不再是"下一代"布局系统，而是Chrome当前使用的布局引擎。LayoutNG 实现了输入和输出的清晰分离——布局输入是不可变的（immutable），布局结果则生成全新的布局片段（Layout Fragment），而不是直接修改输入的布局树。这一设计让布局算法更加简单、可预测，也更容易实现增量布局（只重新计算发生变化的部分）。LayoutNG 还为后续的高级布局特性（如子网格 Subgrid、容器查询 Container Queries）提供了坚实的基础。

> **2025补充：CSS containment 和 content-visibility**：为了帮助浏览器优化渲染性能，CSS引入了两个重要的新特性：
>
> - **`contain` 属性**：通过声明元素的"包含"关系，告诉浏览器该元素的内部变化不会影响外部布局。例如 `contain: layout` 表示元素内部的布局变化不会影响外部，`contain: paint` 表示元素的绘制内容不会超出其边界。`contain: strict`（等同于 `contain: size layout paint style`）是最严格的包含模式，可以让浏览器进行最大程度的优化。
>
> - **`content-visibility: auto`**：这是一个非常强大的性能优化属性。当元素设置了 `content-visibility: auto` 后，如果该元素不在视口内（屏幕可见区域），浏览器会跳过其内容的渲染（包括布局和绘制），直到该元素即将进入视口时才开始渲染。对于长列表或内容丰富的页面，这可以**大幅减少初始渲染时间**。例如：
>
> ```css
> .card {
>   content-visibility: auto;
>   contain-intrinsic-size: 0 200px; /* 提供预估高度，避免滚动条跳动 */
> }
> ```
...
![渲染流水线](/images/browser/rendering-process-part1-html-css-js-to-page/render-pipeline.png)

从图中可以看出，本节内容我们介绍了渲染流程的前三个阶段：DOM 生成、样式计算和布局。要点可大致总结为如下：

- 浏览器不能直接理解 HTML 数据，所以第一步需要将其转换为浏览器能够理解的 DOM 树结构。

- 生成 DOM 树后，还需要根据 CSS 样式表，来计算出 DOM 树所有节点的样式。CSS 的 `@layer` 特性让样式优先级管理更加清晰可控。

- 最后计算 DOM 元素的布局信息，使其都保存在布局树中。Chrome 的 LayoutNG 布局引擎已全面落地，实现了更清晰的输入输出分离。

- 利用 `contain` 和 `content-visibility` 等CSS属性，可以帮助浏览器跳过不必要的渲染工作，显著提升页面性能。