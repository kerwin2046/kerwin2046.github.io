...
description: "有些数据被使用之后，可能就不再需要了，我们把这种数据称为垃圾数据。如果这些垃圾数据一直保存在内存中，那么内存会越用越多，所以我们需要对这些垃圾数据进行回收，以释放有限的内存空间。 ## 不同语言的垃圾回收策略 通常情况下，垃圾数据回收分为手动回收和自动回收两种策略。 如 C/C++ 就是使用手..."
date: "2024-02-08"
author: "Kerwin"
tag: "Frontend"
---
...
![执行到showName函数时的内存模型](/images/browser/garbage-collection-how-garbage-auto-recycled/showName-memory-model.png)
...
![从栈中回收showName执行上下文](/images/browser/garbage-collection-how-garbage-auto-recycled/recycle-showName-execute-context.png)
...
![foo函数执行结束后的内存状态](/images/browser/garbage-collection-how-garbage-auto-recycled/foo-memory-status.png)
...
![新生区要划分为对象区域和空闲区域](/images/browser/garbage-collection-how-garbage-auto-recycled/cenozoic-divide.png)
...
![标记过程](/images/browser/garbage-collection-how-garbage-auto-recycled/mark-process.png)
...
![标记清除过程](/images/browser/garbage-collection-how-garbage-auto-recycled/mark-sweep-process.png)

上面的标记过程和清除过程就是标记-清除算法，不过对一块内存多次执行标记-清除算法后，会产生大量不连续的内存碎片。而碎片过多会导致大对象无法分配到足够的连续内存，于是又产生了另外一种算法——标记-整理（Mark-Compact），这个标记过程仍然与标记-清除算法里的是一样的，但后续步骤不是直接对可回收对象进行清理，而是让所有存活的对象都向一端移动，然后直接清理掉端边界以外的内存。你可以参考下图：

![标记整理过程](/images/browser/garbage-collection-how-garbage-auto-recycled/mark-compact-process.png)
...
![全停顿](/images/browser/garbage-collection-how-garbage-auto-recycled/stop-the-world.png)
...
![增量标记](/images/browser/garbage-collection-how-garbage-auto-recycled/incremental-marking.png)
