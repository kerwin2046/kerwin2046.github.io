---
title: "HTTP3：甩掉TCP、TLS包袱，构建高效网络"
description: "前面两篇文章我们分析了 HTTP / 1 和 HTTP / 2，在 HTTP / 2 出现之前，开发者需要采用很多变通的方式来解决 HTTP / 1 所存在的问题，不过 HTTP / 2 在 2018 年就开始得到了大规模的应用，HTTP / 1 中存在的一大堆缺陷都得到了解决。 HTTP / 2..."
date: "2024-04-07"
author: "Kerwin"
tag: "DevOps"
---
...
![正常情况下的TCP传输数据过程](/images/browser/http3-drop-tcp-tls-build-efficient-network/tcp-transfer-data-process.png)
...
![TCP丢包状态](/images/browser/http3-drop-tcp-tls-build-efficient-network/tcp-packet-loss-status.png)

我们就把在 TCP 传输过程中，由于单个数据包的丢失而造成的阻塞称为 TCP 上的队头阻塞。

那队头阻塞是怎么影响 HTTP / 2 传输的呢？首先我们来看正常情况下 HTTP / 2 是怎么传输多路请求的，为了直观理解，你可以参考下图：

![HTTP/2多路复用](/images/browser/http3-drop-tcp-tls-build-efficient-network/http-2-multiplexing.png)
...
![网络延时](/images/browser/http3-drop-tcp-tls-build-efficient-network/network-delayed.png)
...
![HTTP/2和HTTP/3协议栈](/images/browser/http3-drop-tcp-tls-build-efficient-network/http2-http3-agreement-stack.png)
...
![QUIC协议的多路复用](/images/browser/http3-drop-tcp-tls-build-efficient-network/quic-multiplexing.png)
