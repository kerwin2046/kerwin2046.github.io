...
description: "一个TCP连接过程包括了建立连接、传输数据和断开连接三个阶段。 而HTTP协议，正是建立在TCP连接基础之上的。HTTP是一种允许浏览器向服务器获取资源的协议，是Web的基础，通常由浏览器发起请求，用来获取不同类型的文件，例如HTML文件、CSS文件、JavaScript文件、图片、视频等。此外，..."
date: "2024-01-07"
author: "Kerwin"
tag: "Browser"
---
...
> **2025更新说明**：本文最初基于HTTP/1.1的请求流程编写。截至2025年，HTTP/2已成为主流（支持多路复用、头部压缩等），HTTP/3（基于QUIC协议）也在快速普及。同时，浏览器缓存策略也有了新的发展，如 `Stale-While-Revalidate`、`Cache-Control: immutable` 等现代缓存头，以及Service Worker离线缓存策略。Cookie安全方面，`SameSite` 属性已成为默认要求。以下内容已根据最新进展进行更新。
...
> **2025补充**：需要注意的是，HTTP/1.1和HTTP/2都基于TCP传输，而HTTP/3则基于QUIC（UDP）传输。HTTP/2引入了**多路复用**——在同一个TCP连接上可以并行发送多个请求和响应，不再需要为同一域名建立多个TCP连接。HTTP/3更进一步，使用QUIC协议解决了TCP层面的队头阻塞问题。
...
> **勘误说明**：原文中的 `HTTP1.1` 应为 `HTTP/1.1`，HTTP版本号的正确写法包含斜杠分隔符。
...
> **2025补充：Service Worker缓存**：除了传统的浏览器HTTP缓存之外，现代Web应用还可以使用**Service Worker**来实现更精细的缓存策略。Service Worker是一个运行在浏览器后台的脚本，可以拦截网络请求并从缓存中返回响应，甚至在完全离线的情况下也能提供内容。常见的Service Worker缓存策略包括：Cache First（缓存优先）、Network First（网络优先）、Stale-While-Revalidate（先返回缓存，同时后台更新）等。这使得PWA（渐进式Web应用）能够提供接近原生应用的离线体验。
...
![TCP与HTTP的关系](/images/browser/http-request-flow-why-second-load-faster/tcp-http-relation.png)
...
拿到IP之后，接下来就需要获取端口号了。通常情况下，如果URL没有特别指明端口号，那么HTTP协议默认是80端口，HTTPS协议默认是443端口。
...
> **2025补充**：上述6个连接的限制是针对HTTP/1.1的。在HTTP/2中，由于支持**多路复用**（Multiplexing），浏览器对同一域名通常只建立**1个TCP连接**，所有请求和响应都在这个连接上并行传输，因此不再存在TCP队列等待的问题。在HTTP/3中，QUIC协议进一步优化了这一机制，每个流（stream）都是独立的，一个流上的丢包不会阻塞其他流。
...
![发送请求](/images/browser/http-request-flow-why-second-load-faster/request-to-send.png)
...
![响应数据](/images/browser/http-request-flow-why-second-load-faster/response-to-send.png)
...
![重定向信息](/images/browser/http-request-flow-why-second-load-faster/redirect-data.png)
...
![浏览器缓存处理的过程](/images/browser/http-request-flow-why-second-load-faster/browser-cache.png)
...
> **2025补充：现代缓存策略**：除了传统的 `Cache-Control: max-age` 和 ETag/If-None-Match 机制之外，现代HTTP缓存还引入了以下重要特性：
>
> - **`Stale-While-Revalidate`**：允许浏览器在缓存过期后立即返回旧的缓存内容给用户（保证快速响应），同时在后台异步向服务器请求更新。例如：`Cache-Control: max-age=3600, stale-while-revalidate=60` 表示缓存1小时后，在接下来的60秒内可以先使用过期缓存，同时后台更新。
>
> - **`Cache-Control: immutable`**：告诉浏览器这个资源永远不会改变（通常配合文件名中的哈希使用），即使用户手动刷新页面，浏览器也不需要向服务器发送验证请求。这对于带有内容哈希的静态资源（如 `app.a1b2c3.js`）非常有效。
>
> - **Service Worker缓存**：通过Service Worker，开发者可以完全控制缓存策略，实现离线访问、缓存优先或网络优先等策略，这比传统的HTTP缓存更加灵活和强大。
...
![Cookie流程](/images/browser/http-request-flow-why-second-load-faster/cookie-process.png)
...
> **2025补充：Cookie的SameSite属性**：为了防范CSRF（跨站请求伪造）攻击，现代浏览器要求Cookie设置 `SameSite` 属性。自Chrome 80起，未显式设置SameSite的Cookie默认被视为 `SameSite=Lax`，这意味着Cookie不会在跨站的子请求（如加载图片或iframe）中发送，只有在用户主动导航到目标站点时才会携带。SameSite有三个值：
>
> - **`Strict`**：Cookie只在同站请求中发送，最严格。
> - **`Lax`**（默认值）：在跨站的顶级导航（如点击链接）中会发送Cookie，但在跨站的子请求中不会发送。
> - **`None`**：Cookie在所有请求中都会发送，但必须同时设置 `Secure` 属性（仅通过HTTPS发送）。
>
> 这一变化显著提升了Web应用的安全性，但也要求开发者在设置Cookie时明确声明其跨站行为。
...
![HTTP请求的各阶段](/images/browser/http-request-flow-why-second-load-faster/http-process.png)

从图中可以看到，浏览器中的HTTP请求从发起到结束一共经历了如下八个阶段：构建请求、查找缓存、准备IP和端口、等待TCP队列、建立TCP连接、发起HTTP请求、服务器处理请求、服务器返回请求和断开连接。

> **2025补充**：在HTTP/2和HTTP/3的环境下，上述流程中的"等待TCP队列"这一步已基本消除。HTTP/2通过多路复用在单个TCP连接上并行传输所有请求；HTTP/3更进一步使用QUIC协议，连建立连接的延迟也大幅降低（支持0-RTT重连）。
