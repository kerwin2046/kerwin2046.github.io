...
description: "上一篇文章我们聊了 HTTP / 1.1 的发展史，虽然 HTTP / 1.1 已经做了大量的优化，但是依然存在很多性能瓶颈，依然不能满足我们日益变化的新需求，所以就有了我们今天要聊的 HTTP / 2。 本文我们依然从需求的层面来谈，先分析 HTTP / 1.1 存在哪些问题，然后再来分析 HT..."
date: "2024-04-04"
author: "Kerwin"
tag: "DevOps"
---
...
> **2025 更新说明**：本文最初撰写于 HTTP/2 刚推广阶段。截至 2025 年，HTTP/2 已成为互联网上的主导协议，全球超过 65% 的网站已部署 HTTP/2。同时需要注意：**HTTP/2 Server Push 已被弃用**（Chrome 于 2022 年移除支持，其他主流浏览器也已跟进），取而代之的是 103 Early Hints 机制。此外，HTTP/2 仍然受到 TCP 层队头阻塞的影响，这一问题由 HTTP/3（基于 QUIC）解决。
...
![HTTP/1.1的资源下载方式](/images/browser/http2-how-to-improve-network-speed/http-1.1-resource-download-mode.png)
...
![HTTP/2的多路复用](/images/browser/http2-how-to-improve-network-speed/http-2-multiplexing.png)
...
![HTTP/2的多路复用原理](/images/browser/http2-how-to-improve-network-speed/http-2-multiplexing-theory.png)
...
> **2025 补充：关于优先级机制的演进**
>
> HTTP/2 最初定义的优先级模型基于**依赖树和权重**，但该方案在实际实现中存在诸多问题：不同浏览器和服务器对优先级的理解和实现差异很大，导致优先级信号经常被忽略或错误处理。
>
> 为此，IETF 发布了 **RFC 9218（Extensible Prioritization Scheme for HTTP）**，引入了一种更简洁、更具可操作性的优先级方案。新方案使用 `Priority` 请求头和 `PRIORITY_UPDATE` 帧，通过 `urgency`（紧急程度，0-7）和 `incremental`（是否增量传输）两个参数来表达优先级。这一方案同时适用于 HTTP/2 和 HTTP/3，解决了旧方案实现不一致的问题。

### 2.服务器推送（已弃用）

> **重要更新（2025）**：HTTP/2 Server Push 已被**弃用并从主流浏览器中移除**。Chrome 于 2022 年（Chrome 106）正式移除了对 Server Push 的支持，Firefox、Safari 和 Edge 也已跟进。弃用的主要原因包括：
>
> 1. **缓存失效问题**：服务器无法准确判断浏览器是否已经缓存了某个资源，导致频繁推送浏览器已有的资源，反而浪费了带宽。
> 2. **实现复杂性**：正确配置 Server Push 非常困难，大多数部署效果不佳甚至适得其反。
> 3. **与预加载机制冲突**：浏览器自身的预加载扫描器（preload scanner）已经能够高效发现和请求关键资源。
>
> **推荐替代方案：103 Early Hints**。服务器可以在最终响应生成前先返回 `103 Early Hints` 响应，在其中通过 `Link` 头部告知浏览器需要预加载的资源。这种方式更加灵活，且能与浏览器缓存正确配合。
...
HTTP / 2 使用的头部压缩算法称为 **HPACK**，它通过维护一张静态表和一张动态表来实现高效压缩。对于常见的头部字段（如 `:method: GET`、`:path: /`），直接使用索引号代替；对于重复出现的自定义头部，会加入动态表中以便后续引用。这在请求大量资源时效果尤为显著。

## HTTP/2 的局限性（2025 补充）

尽管 HTTP/2 通过多路复用解决了 HTTP 层面的队头阻塞问题，但它依然存在一个根本性的局限——**TCP 层的队头阻塞**。

由于 HTTP/2 的所有流都共享同一个 TCP 连接，当 TCP 层发生丢包时，整个连接上的所有流都必须等待丢失的数据包被重传。也就是说，一个数据包的丢失会阻塞该连接上所有正在传输的请求。有研究表明，当丢包率达到 2% 时，HTTP/1.1（多个 TCP 连接）的性能反而可能优于 HTTP/2（单个 TCP 连接）。

这个问题是 TCP 协议本身的特性决定的，无法在 HTTP/2 层面解决。正是这一限制催生了 HTTP/3——它基于 QUIC 协议（运行在 UDP 之上），每个流拥有独立的丢包恢复机制，从根本上解决了队头阻塞问题。
...
接下来我们分析了 HTTP / 2 是如何采用多路复用机制来解决这些问题的。多路复用是通过在协议栈中添加二进制分帧层来实现的，有了二进制分帧层还能够实现请求的优先级、头部压缩等特性，从而大大提升了文件传输效率。

HTTP / 2 协议规范于 2015 年 5 月正式发布，在那之后，该协议已在互联网和万维网上得到了广泛的实现和部署。截至 2025 年，HTTP/2 已成为互联网的主导协议，全球超过 65% 的网站使用 HTTP/2 提供服务。使用 HTTP / 2 能带来 20% ~ 60% 的效率提升，至于 20% 还是 60% 要看优化的程度。

> **2025 关键总结**：
> - HTTP/2 已经是互联网主流协议，广泛部署于各大网站和 CDN。
> - HTTP/2 Server Push 已被弃用并从浏览器中移除，应使用 **103 Early Hints** 作为替代。
> - HTTP/2 的优先级机制已通过 **RFC 9218** 进行了改进，新方案更简洁且实现更一致。
> - HTTP/2 仍受 **TCP 队头阻塞**影响，这一根本性问题由 HTTP/3（基于 QUIC）解决。
> - 对于追求极致网络性能的场景，应同时考虑部署 HTTP/3。