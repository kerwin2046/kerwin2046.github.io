...
description: "在前面几篇文章中，我们介绍了消息队列，并结合消息队列介绍了两种典型的 WebAPI——setTimeout 和 XMLHttpRequest，通过这两个 WebAPI 我们搞清楚了浏览器的消息循环系统是怎么工作的。不过随着浏览器的应用领域越来越广泛，消息队列中这种粗时间颗粒度的任务已经不能胜任部分领..."
date: "2024-02-24"
author: "Kerwin"
tag: "Frontend"
---
...
> **2025 更新说明：** 本文在原有内容基础上，补充了 `queueMicrotask()` 标准 API、完整的微任务来源列表、事件循环完整执行顺序、以及包含 `setTimeout` / `Promise.then` / `queueMicrotask` / `requestAnimationFrame` / `await` 的实际执行顺序代码示例，帮助你建立更加精确的事件循环心智模型。
...
![performance记录](/images/browser/macro-task-micro-task-different-treatment/performance-log.png)
...
我们先来看看微任务是怎么产生的？在现代浏览器里面，产生微任务的方式已经比较丰富了，下面是完整的微任务来源列表：

**1. MutationObserver**

使用 MutationObserver 监控某个 DOM 节点，然后再通过 JavaScript 来修改这个节点，或者为这个节点添加、删除部分子节点，当 DOM 节点发生变化时，就会产生 DOM 变化记录的微任务。

**2. Promise 的回调（then / catch / finally）**

当调用 Promise.resolve() 或者 Promise.reject() 的时候，也会产生微任务。更准确地说，Promise 的 `.then()`、`.catch()`、`.finally()` 注册的回调函数都是以微任务的形式被执行的。

**3. `queueMicrotask()`**

这是一个标准化的微任务 API，可以直接将一个回调函数添加到微任务队列中。在 `queueMicrotask()` 出现之前，开发者往往需要用 `Promise.resolve().then(fn)` 这种间接方式来手动添加微任务，语义不够清晰。现在有了 `queueMicrotask()`，添加微任务变得直观且高效：

```js
queueMicrotask(() => {
  console.log('通过 queueMicrotask 添加的微任务');
});
```

**4. async / await 中的 await 表达式**

在 `async` 函数中，每个 `await` 表达式都会创建微任务。当执行到 `await` 时，JavaScript 引擎会暂停当前协程，并将 `await` 后续的代码注册为微任务。下面的代码：

```js
async function foo() {
  console.log('a');
  await someValue;
  console.log('b'); // 这一行实际上是作为微任务执行的
}
```
...
function foo() {
  console.log('a');
  Promise.resolve(someValue).then(() => {
    console.log('b');
  });
}
```

**5. `process.nextTick()`（Node.js 环境）**

在 Node.js 环境中，`process.nextTick()` 也会产生微任务，并且它的优先级甚至高于 Promise 微任务。需要注意的是，`process.nextTick()` 是 Node.js 特有的 API，浏览器环境中不可用。

通过以上这些方式产生的微任务，都会被 JavaScript 引擎按照顺序保存到微任务队列中。
...
![微任务添加和执行流程示意图1](/images/browser/macro-task-micro-task-different-treatment/micro-task-execute-process-1.png)

![微任务添加和执行流程示意图2](/images/browser/macro-task-micro-task-different-treatment/micro-task-execute-process-2.png)
...
理解了宏任务和微任务之后，我们来梳理一下浏览器中**一轮完整的事件循环**到底经历了哪些阶段。根据 WHATWG 规范和浏览器的实际实现，一轮事件循环的完整顺序如下：

1. **执行一个宏任务**（从宏任务队列中取出一个任务执行）。
2. **执行所有微任务**（清空微任务队列，包括执行过程中新产生的微任务）。
3. **判断是否需要渲染更新**（浏览器会根据屏幕刷新率、页面是否可见等因素来决定）。
4. 如果需要渲染更新，则依次执行：
   - 执行 `requestAnimationFrame` 回调。
   - 执行样式计算、布局、绘制等渲染步骤。
5. 如果渲染完成后还有空闲时间，则执行 `requestIdleCallback` 回调。
6. **回到第 1 步**，开始下一轮事件循环。
...
while (true) {
  ...
  // 3. 判断是否需要渲染
  if (shouldRender()) {
    // 4a. 执行 requestAnimationFrame 回调
    executeRAFCallbacks();
    // 4b. 渲染（样式计算 → 布局 → 绘制）
    render();
  }
...
理解这个完整的执行顺序非常重要，它解释了为什么微任务总是在宏任务之后、渲染之前执行，也解释了 `requestAnimationFrame` 为什么是做动画的最佳选择——因为它的执行时机恰好在渲染之前。

### 实战：验证执行顺序的代码示例

下面我们通过一段代码来验证上述执行顺序。请你仔细思考这段代码的输出顺序：

```js
console.log('1. 同步代码（宏任务开始）');

setTimeout(() => {
  console.log('6. setTimeout 回调（下一个宏任务）');
}, 0);
...
6. setTimeout 回调（下一个宏任务）
```

我们来分析这个执行过程：

1. **同步代码执行**：先打印 `1` 和 `2`，这属于当前宏任务中的同步代码。
2. **微任务执行**：同步代码执行完毕后，进入微任务检查点。按照入队顺序，先执行 `Promise.then` 的回调打印 `3`，再执行 `queueMicrotask` 的回调打印 `4`。
3. **requestAnimationFrame 执行**：微任务清空后，如果需要渲染，则执行 rAF 回调，打印 `5`。
4. **下一个宏任务**：渲染完成后，从宏任务队列中取出 `setTimeout` 的回调执行，打印 `6`。

再来看一个涉及 `async / await` 的更复杂的例子：

```js
async function asyncFn() {
  console.log('2. async 函数同步部分');
...
Promise.resolve().then(() => {
  ...
  console.log('6. Promise.then 链式调用');
});
...
- `async` 函数中 `await` 之前的代码是同步执行的，所以 `2` 紧跟 `1` 打印。
- `await` 之后的代码被转换为微任务，排在微任务队列中。
- `Promise.then` 和 `queueMicrotask` 按入队顺序执行。
- `await` 产生的微任务在 `Promise.resolve().then()` 和 `queueMicrotask()` 之后入队（因为 `await Promise.resolve()` 需要等待 Promise resolve，resolve 回调本身也是一个微任务），所以 `5` 在 `3` 和 `4` 之后打印。
- `6` 是第一个 `.then()` 执行完后注册的新微任务，排在队列最后。
...
我们还梳理了完整的微任务来源——包括 **MutationObserver**、**Promise.then / catch / finally**、**`queueMicrotask()`**、**`async / await` 中的 `await`** 以及 Node.js 环境中的 **`process.nextTick()`**。

我们深入分析了**完整的事件循环执行顺序**：宏任务 → 所有微任务 → requestAnimationFrame → 渲染 → requestIdleCallback → 下一个宏任务。理解这个顺序对于编写高性能的 Web 应用至关重要。
