---
title: "Webapi：XMLHttpRequest 是怎么实现的"
description: "在上一篇文章中我们介绍了 setTimeout 是如何结合渲染进程的循环系统工作的，那本篇文章我们就继续介绍另外一种类型的 WebAPI——XMLHttpRequest。 自从网页中引入了 JavaScript，我们就可以操作 DOM 树中任意一个节点，例如隐藏 / 显示节点、改变颜色、获得或改变..."
date: "2024-02-21"
author: "Kerwin"
tag: "Frontend"
---
...
> **2025 更新说明**：本文在原有内容基础上，新增了 Fetch API 作为现代替代方案的详细介绍（包括 Promise 化设计、Request/Response 对象、AbortController、ReadableStream 等），补充了 Beacon API 的说明，并对 XMLHttpRequest 在现代开发中的定位做了更新。
...
![消息循环系统调用栈记录](/images/browser/webapi-how-xmlhttprequest-implemented/call-stack-log.png)
...
![XMLHttpRequest工作流程图](/images/browser/webapi-how-xmlhttprequest-implemented/XMLHttpRequest-process.png)
...
![responseType描述](/images/browser/webapi-how-xmlhttprequest-implemented/response-type-description.png)
...
![HTTPS混合内容警告](/images/browser/webapi-how-xmlhttprequest-implemented/https-mixin-warn.png)
...
![使用XMLHttpRequest混合资源失效](/images/browser/webapi-how-xmlhttprequest-implemented/XMLHttpRequest-mixin-error.png)

## 现代替代方案：Fetch API（2025 补充）

XMLHttpRequest 诞生于 Web 的早期时代，虽然功能强大，但其基于事件回调的 API 设计在现代开发中显得繁琐。**Fetch API** 是浏览器提供的现代网络请求接口，已经成为当前前端开发中发起 HTTP 请求的首选方案。

### Fetch API 的核心设计

Fetch API 采用了 **基于 Promise 的设计**，与现代 JavaScript 的 async/await 语法完美配合，代码更简洁、逻辑更清晰：

```js
// 使用 Fetch API 发起请求（async/await 写法）
...
对比之前的 XMLHttpRequest 代码，你会发现 Fetch 的写法清晰了很多——没有回调地狱，没有复杂的状态判断，错误处理也更加直观。

### Request 与 Response 对象

Fetch API 引入了标准化的 **Request** 和 **Response** 对象，让请求和响应都成为了可编程的一等公民：
...
const request = new Request('https://api.example.com/data', {
  ...
  body: JSON.stringify({ name: '极客时间' })
})

const response = await fetch(request)
```

**Headers 对象** 提供了一组方法来操作 HTTP 请求头和响应头：
...
XMLHttpRequest 可以通过 `xhr.abort()` 来取消请求，Fetch API 则通过 **AbortController** 实现同样的功能，而且更加通用：
...
`AbortController` 的优势在于它不局限于 Fetch，还可以用于取消事件监听、流操作等任何支持 `AbortSignal` 的 API。

### ReadableStream：流式读取响应

Fetch API 的 Response 对象的 `body` 属性是一个 **ReadableStream**，支持流式读取数据。这对于处理大文件下载或实时数据流非常有用：
...
### Fetch API 的默认行为和注意事项

使用 Fetch API 时有几个重要的默认行为需要了解：

1. **Cookie 发送策略**：`fetch()` 默认的 `credentials` 值为 `'same-origin'`，即只有同源请求才会自动携带 Cookie。如果需要在跨域请求中携带 Cookie，需要显式设置 `credentials: 'include'`：
...
2. **不透明响应（Opaque Response）**：当使用 `mode: 'no-cors'` 发起跨域请求时，返回的响应是不透明的（opaque），JavaScript 无法读取响应内容、状态码或响应头。这种模式主要用于 Service Worker 缓存场景。

3. **HTTP 错误不会 reject**：与 XMLHttpRequest 不同，Fetch 在收到 HTTP 错误状态码（如 404、500）时**不会**抛出异常，Promise 仍然会 resolve。你需要自己检查 `response.ok` 或 `response.status` 来判断请求是否成功。

4. **不支持同步请求**：Fetch API 没有同步模式，所有请求都是异步的（这实际上是一个优点，同步请求会阻塞主线程）。
...
| 流式读取 | 有限支持 | `ReadableStream` 原生支持 |
| Cookie 默认行为 | 同源自动携带 | `credentials: 'same-origin'` |
| HTTP 错误处理 | 触发 `onload`，需检查 `status` | Promise resolve，需检查 `response.ok` |
| 上传进度 | `xhr.upload.onprogress` | 原生不支持（需借助自定义逻辑） |
| 同步请求 | 支持（不推荐） | 不支持 |
| Service Worker | 不可用 | 完全支持 |

## Beacon API：发送分析数据的最佳方式（2025 补充）

在 Web 应用中，常见的一个需求是在页面卸载时发送分析数据或日志。使用 XMLHttpRequest 或 Fetch 在 `unload`/`beforeunload` 事件中发送数据并不可靠，因为浏览器可能会在请求完成前就关闭了页面。

**Beacon API**（`navigator.sendBeacon()`）就是为了解决这个问题而设计的，它是一种"发出即忘（fire-and-forget）"的请求方式：
...
- **可靠性高**：浏览器会保证在页面卸载前将数据发送出去，不会因为页面关闭而丢失请求。
- **非阻塞**：不会延迟页面卸载或影响下一个页面的加载性能。
- **简单易用**：只有一个方法，无需处理回调或 Promise。
- **限制**：只能发送 POST 请求，且无法获取服务器响应。

## XMLHttpRequest 在现代开发中的定位（2025 补充）

虽然 Fetch API 已经成为主流，但 XMLHttpRequest 并没有完全退出历史舞台。以下场景中 XHR 仍然有其存在价值：

- **上传进度监控**：`xhr.upload.onprogress` 事件可以方便地获取上传进度百分比，Fetch API 原生并不支持上传进度（不过目前已有关于 Fetch Upload Streaming 的提案在推进）。
- **遗留项目维护**：大量现有代码库仍然基于 XMLHttpRequest 构建，包括一些流行的库（如早期版本的 Axios 在浏览器端就使用了 XHR）。
- **同步请求**：极少数场景可能需要同步请求（如 Web Worker 中的同步 XHR），Fetch API 完全不支持同步模式。

不过，对于新项目的开发，强烈建议优先使用 Fetch API，并结合 `async/await` 语法来编写清晰、现代的异步代码。
...
> **2025 总结补充**：在现代前端开发中，Fetch API 已经取代 XMLHttpRequest 成为网络请求的首选方案。它基于 Promise 的设计更符合现代 JavaScript 的编程范式，配合 `async/await`、`AbortController`、`ReadableStream` 等能力，能够更优雅地处理各种网络请求场景。同时，Beacon API 为数据上报等"发出即忘"的需求提供了专门的解决方案。理解 XMLHttpRequest 的工作原理仍然很有价值——它帮助你理解浏览器的网络请求机制，而这个机制是所有网络 API 共通的。