...
description: "在上一篇文章中我们详细介绍了 DOM 的生成过程，并结合具体例子分析了 JavaScript 是如何阻塞 DOM 生成的。那本文我们就继续深入聊聊渲染流水线中的 CSS。因为 CSS 是页面中非常重要的资源，它决定了页面最终显示出来的效果，并影响着用户对整个网站的第一体验。所以，搞清楚浏览器中的 C..."
date: "2024-03-11"
author: "Kerwin"
tag: "Frontend"
---
...
> **2025 更新说明**：本文在原有内容基础上，新增了 `<link rel="preload">`、`fetchpriority` 资源优先级、`modulepreload`（ES 模块预加载）、HTTP 103 Early Hints、Core Web Vitals 中 LCP 与 CSS 的关系、以及 `content-visibility: auto` 等现代性能优化手段的说明。
...
![含有CSS的页面渲染流水线](/images/browser/rendering-pipeline-how-css-affects-white-screen/css-render-line.png)
...
![含有JavaScript和CSS的页面渲染流水线](/images/browser/rendering-pipeline-how-css-affects-white-screen/javascript-css-render-line.png)
...
![含有JavaScript文件和CSS文件页面的渲染流水线](/images/browser/rendering-pipeline-how-css-affects-white-screen/javascript-css-file-render-line.png)
...
### Core Web Vitals 视角：CSS 与 LCP 的关系

在讨论优化策略之前，我们需要了解一个重要的性能指标框架——**Core Web Vitals（核心 Web 指标）**。其中 **LCP（Largest Contentful Paint，最大内容绘制）** 直接衡量了用户在打开页面时看到主要内容的速度，Google 将其作为搜索排名的参考因素之一。

CSS 加载对 LCP 的影响是直接且显著的：

- **CSS 是渲染阻塞资源**：浏览器在 CSSOM 构建完成前不会进行首次渲染，因此 CSS 下载和解析时间直接叠加到 LCP 时间上。
- **字体加载**：如果 CSS 中引用了自定义字体（`@font-face`），字体文件的下载也可能导致文本内容的延迟显示，进而影响 LCP。
- **关键 CSS 路径**：只有构建布局树所需的 CSS 才是真正的关键资源，其余 CSS 可以延迟加载。

因此，优化 CSS 加载就是优化 LCP 的核心手段之一。
...
除了上述经典策略之外，现代浏览器和 Web 标准还提供了一系列更精细的优化手段：

#### 1. `<link rel="preload">`：显式预加载关键资源

`preload` 是一种声明式的资源获取请求，它告诉浏览器"这个资源在当前页面中肯定需要，请尽早开始获取它"。对于关键 CSS 和字体，这特别有用：
...
`preload` 会将资源的下载优先级提升，使其尽早开始传输，但不会阻塞页面渲染——它只是"提前获取"，实际的应用还是由对应的 `<link rel="stylesheet">` 或 CSS `@font-face` 触发。

#### 2. `fetchpriority`：精细控制资源优先级

`fetchpriority` 属性允许开发者在浏览器默认优先级的基础上进行微调：
...
<!-- 非关键 CSS 低优先级 -->
<link rel="stylesheet" href="below-fold.css" fetchpriority="low" media="print" onload="this.media='all'">

<!-- 关键图片（LCP 元素）高优先级 -->
<img src="hero.jpg" fetchpriority="high" alt="首屏大图">
```

取值为 `high`、`low` 或 `auto`（默认）。配合 `preload` 使用时效果更佳，特别是当页面中有多个预加载资源时，`fetchpriority` 可以帮助浏览器优先处理最关键的那些。

#### 3. `modulepreload`：ES 模块预加载

对于使用 ES Module 的项目，`modulepreload` 可以提前加载和编译 JavaScript 模块，避免模块依赖链的瀑布式请求：
...
与 `preload` 不同，`modulepreload` 会在下载后立即进行解析和编译，进一步减少模块执行时的延迟。

#### 4. HTTP 103 Early Hints：服务器提前推送预加载提示

**103 Early Hints** 是一种 HTTP 状态码，允许服务器在最终响应（200）准备好之前，提前向浏览器发送资源预加载提示。这在服务器需要较长时间生成动态页面时特别有效：

```
HTTP/1.1 103 Early Hints
Link: </critical.css>; rel=preload; as=style
Link: </main-font.woff2>; rel=preload; as=font; crossorigin
...
浏览器在收到 103 响应后就会立即开始预加载指定资源，而不用等待最终的 HTML 响应。这意味着在服务器处理请求的这段"思考时间"里，浏览器已经在并行下载关键资源了。Chrome 从 103 版本开始支持 Early Hints。

#### 5. `content-visibility: auto`：渲染优化

`content-visibility` 是一个 CSS 属性，可以显著优化渲染性能，特别是对于首屏之外的内容：

```css
/* 对于页面底部不在视口中的内容区块 */
.below-fold-section {
  content-visibility: auto;
  contain-intrinsic-size: 0 500px; /* 提供预估尺寸，避免滚动条跳动 */
}
```

当元素设置了 `content-visibility: auto` 时，如果该元素不在视口范围内（即用户看不到），浏览器会跳过该元素子树的渲染工作（包括布局、绘制等），等到元素即将进入视口时才进行渲染。这对于长页面的首次渲染性能有显著提升：

- **跳过屏幕外内容的布局和绘制**：减少首次渲染的工作量，加快 LCP。
- **自动管理**：浏览器自动判断元素是否在视口内，开发者无需手动管理。
- **配合 `contain-intrinsic-size`**：提供预估尺寸，避免因为跳过渲染导致页面高度计算错误，引起滚动条跳动。
...
- 在 Core Web Vitals 时代，CSS 优化对 LCP 的影响尤为关键。善用 `preload`、`fetchpriority`、`modulepreload`、103 Early Hints 等现代机制，配合 `content-visibility: auto` 减少首屏外内容的渲染开销，可以显著缩短白屏时间，提升用户体验和搜索引擎排名。