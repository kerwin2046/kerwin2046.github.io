...
description: "\"在浏览器里，从输入URL到页面展示，这中间发生了什么？\"，这是一道经典的面试题，能比较全面地考察应聘者知识的掌握程度，其中涉及到了网络、操作系统、Web等一系列的知识。所以我在面试应聘者时也必问这道题，但遗憾的是大多数人只能回答其中部分零散的知识点，并不能将这些知识点串联成线，无法系统而又全面地回..."
date: "2024-01-10"
author: "Kerwin"
tag: "Browser"
---
...
> **2025更新说明**：本文最初编写时Chrome的导航优化手段相对有限。截至2025年，浏览器在导航阶段引入了大量现代优化技术，包括：`<link rel="preconnect">` 提前建立连接、`<link rel="prefetch">` 预获取资源、Early Hints（103状态码）让服务器在最终响应前提示浏览器预加载资源、推测性预渲染（Speculative Prerendering）等。同时，Chrome的站点隔离和进程分配策略也更加精细。以下内容已根据最新进展进行更新。
...
![从输入URL到页面展示完整流程示意图](/images/browser/navigation-flow-from-url-to-page/input-url-process.png)
...
![关键字搜索](/images/browser/navigation-flow-from-url-to-page/keyword-search.png)
...
> **2025补充：现代导航优化技术**：在上述传统流程之外，现代浏览器和Web标准提供了多种手段来加速导航过程：
>
> - **`<link rel="preconnect">`**：提示浏览器提前与指定源建立连接（包括DNS解析、TCP握手和TLS协商），这样当真正需要请求该源的资源时，连接已经就绪。
>
> - **`<link rel="prefetch">`**：提示浏览器在空闲时预获取将来可能用到的资源（如下一个页面的HTML或关键资源），并将其存入缓存。
>
> - **`<link rel="prerender">` / Speculation Rules API**：Chrome支持通过Speculation Rules API进行**推测性预渲染**，即在用户实际点击链接之前，就在后台完整渲染目标页面。当用户真正点击时，页面几乎可以"瞬间"展示。
>
> - **Early Hints（103状态码）**：服务器在处理请求的过程中，可以先发送一个103状态码响应，提示浏览器开始预加载关键资源（如CSS、字体），而不必等待最终的200响应。这有效利用了服务器的"思考时间"。
>
> - **推测性连接（Speculative Connections）**：当用户将鼠标悬停在链接上或开始输入URL时，Chrome就会推测性地开始DNS预解析和TCP预连接，以减少后续的等待时间。
...
![响应头重定向](/images/browser/navigation-flow-from-url-to-page/response-data-redirect.png)
...
![响应头200](/images/browser/navigation-flow-from-url-to-page/response-data-200.png)
...
![响应头类型html](/images/browser/navigation-flow-from-url-to-page/response-data-type-html.png)
...
![响应头类型stream](/images/browser/navigation-flow-from-url-to-page/response-data-type-stream.png)
...
![相同的渲染进程](/images/browser/navigation-flow-from-url-to-page/render-process-equal.png)
...
> **2025更新：站点定义的演进**：Chrome对"同一站点"的定义已经历了重要变化。随着隐私保护的加强，Chrome引入了**站点分区（Site Partitioning）**机制。在最新的实现中，存储和Cookie等状态数据不仅按站点隔离，还会考虑**顶级站点**（即地址栏显示的站点）。这意味着，同一个第三方站点在不同的顶级站点下，会被视为不同的分区，无法共享Cookie和存储数据。这一变化是Chrome逐步淘汰第三方Cookie的重要一步。
...
![不同的渲染进程](/images/browser/navigation-flow-from-url-to-page/render-process-different.png)
...
![提交文档后更新状态](/images/browser/navigation-flow-from-url-to-page/update-status.png)
...
![渲染完成](/images/browser/navigation-flow-from-url-to-page/render-finish.png)
...
- `如果没有，网络进程向web服务器发起http请求（网络请求），请求流程如下：`
  ...
  - 进行DNS解析，获取服务器IP地址。端口号则由协议决定（HTTP默认80，HTTPS默认443），如果URL中显式指定了端口号则使用指定的端口。
...
- 现代浏览器通过 preconnect、prefetch、Early Hints（103）、推测性预渲染等技术，可以显著减少导航过程中的等待时间。
