...
description: "谈及浏览器中的网络，就避不开 HTTP。我们知道 HTTP 是浏览器中最重要且使用最多的协议，是浏览器和服务器之间的通信语言，也是互联网的基石。而随着浏览器的发展，HTTP 为了能适应新的形式也在持续进化，我们认为学习 HTTP 的最佳途径就是了解其发展史，所以接下来的三篇文章中，我会从浏览器发展的..."
date: "2024-04-01"
author: "Kerwin"
tag: "DevOps"
---
...
> **2025 更新说明**：本文最初撰写时 HTTP/1.1 仍是主流协议。截至 2025 年，HTTP/2 和 HTTP/3 已被大规模采用，但 HTTP/1.1 仍然作为广泛使用的回退协议存在——当浏览器或服务器不支持更高版本时，HTTP/1.1 依然是默认的通信方式。理解 HTTP/1.1 的演进和瓶颈，对于理解后续协议的设计动机至关重要。
...
![HTTP/0.9请求流程](/images/browser/http1-performance-optimization/http-0.9-request-process.png)
...
![HTTP/1.0请求流程](/images/browser/http1-performance-optimization/http-1.0-request-process.png)
...
![HTTP/1.0通信图](/images/browser/http1-performance-optimization/http-1.0-communication.png)
...
![HTTP/1.1通信图](/images/browser/http1-performance-optimization/http-1.1-communication.png)
...
> **注意**：这里的"每个域名 6 个 TCP 连接"限制是 **HTTP/1.1 特有的**。由于 HTTP/1.1 在每个连接上同一时刻只能处理一个请求-响应对，浏览器不得不通过并发多个连接来提高下载效率。到了 HTTP/2，由于引入了多路复用机制，一个域名只需要一个 TCP 连接即可并行传输所有请求，因此这个 6 连接的限制不再适用。
...
> **2025 补充**：HTTP 管线化（Pipelining）从未被广泛采用，目前几乎所有主流浏览器都已默认禁用该特性。其失败的主要原因包括：（1）服务器必须严格按请求顺序响应，导致队头阻塞问题并未真正解决；（2）中间代理服务器的兼容性问题严重；（3）错误处理复杂且不一致。HTTP/2 的多路复用机制才是队头阻塞问题的真正解决方案。
...
## 现代 HTTP/1.1 性能优化技术（2025 补充）

虽然 HTTP/2 和 HTTP/3 已成为主流，但在 HTTP/1.1 仍被使用的场景下，现代浏览器引入了一系列**资源提示（Resource Hints）**机制来进一步优化性能：

### 1. 资源提示（Resource Hints）

- **`<link rel="preload">`**：告诉浏览器当前页面一定会用到的资源（如关键 CSS、字体文件），让浏览器尽早发起请求。与普通加载不同的是，preload 会以高优先级并行下载资源，不会阻塞页面渲染。

- **`<link rel="preconnect">`**：提前与目标域名建立连接（包括 DNS 解析、TCP 握手、TLS 协商），这样当后续真正请求该域名资源时可以节省连接建立的时间。特别适用于与第三方域名（CDN、API 服务器、字体服务等）的通信。

- **`<link rel="prefetch">`**：提示浏览器在空闲时预获取将来导航可能需要的资源，优先级较低，适合预加载用户可能接下来访问的页面资源。

- **`<link rel="dns-prefetch">`**：仅执行 DNS 解析，开销最小，适用于页面中引用了大量外部域名的情况。

### 2. 103 Early Hints

HTTP 103 Early Hints 是一个相对较新的状态码（RFC 8297），允许服务器在最终响应生成完成之前，先发送一个**提示性响应**。服务器可以在 103 响应中包含 `Link` 头部，告诉浏览器可以提前开始预加载或预连接某些资源。

例如，当服务器还在处理 HTML 页面的动态生成时，可以先返回：
...
浏览器收到 103 响应后，会立即开始获取这些资源，而不必等到最终的 200 响应。这对于减少关键资源的加载延迟非常有效，是 HTTP/2 Server Push 被弃用后的推荐替代方案。
...
但随着技术和需求的发展，人们对文件传输的速度要求越来越高，故又基于 HTTP / 1.0 推出了 HTTP / 1.1，增加了持久连接方法来提升连接效率，同时还尝试使用管线化技术提升效率（不过由于各种原因，管线化技术最终被各大厂商放弃了，且从未被广泛采用）。除此之外，HTTP / 1.1 还引入了 Cookie、虚拟主机的支持、对动态内容的支持等特性。

虽然 HTTP / 1.1 在 HTTP / 1.0 的基础之上做了大量的优化，但是由于一些效率问题始终很难解决，所以最终还是被 HTTP / 2 所取代了，这就是我们下一篇要介绍的内容了。

> **2025 视角**：尽管 HTTP/2 和 HTTP/3 已是主流，HTTP/1.1 并未完全退出历史舞台。它仍然是许多老旧系统、内部服务以及不支持更高版本协议的场景中的默认回退协议。同时，现代浏览器通过资源提示（preload、preconnect、prefetch、dns-prefetch）和 103 Early Hints 等机制，为 HTTP/1.1 的性能优化提供了额外的手段。理解 HTTP/1.1 的局限性，有助于我们更好地理解 HTTP/2 和 HTTP/3 的设计动机。