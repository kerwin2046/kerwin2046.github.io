...
declare -A FILES=(
  [2010]="$TOOLS/a38c904e-e4d3-4a89-bdd9-a1ad0bfc6b86.txt"
  [2011]="$TOOLS/504d4309-c654-4a8e-af01-16981c113d7f.txt"
  [2012]="$TOOLS/9b163a66-9c0c-4607-b36f-1700893858cf.txt"
...
for y in $(seq 2010 2026); do
  f="${FILES[$y]}"
  if [[ -f "$f" ]]; then
    python3 "$SCRIPT_DIR/scrape_translation_writing.py" "$f" "$y"
  else
    echo "MISSING $y: $f"
  fi
done
